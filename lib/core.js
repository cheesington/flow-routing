  ...
  function doLayout() {
    if (layoutType === "blaze-to-react") {
      ...
      // Render using BlazeLayout
		if (!FlowRouter.Renderer) {
			throw new Meteor.Error('useraccounts:flow-routing', 'Couldn\'t render using Blaze.  Maybe missing blaze-html-templates package?');
		}
      FlowRouter.Renderer.render(layoutTemplate, layoutRegions);
    }
  }
